...
2. Change `ACTIVE_THEME`:
   ```lua
   local ACTIVE_THEME = "your-theme-name"
   ```
3. Save and restart Neovim. `:Lazy sync` is only needed when adding a new
   plugin-backed theme.

## DMS / Matugen Theme

Set `ACTIVE_THEME` to `"dms"` to follow the active DMS palette. The editable
source is `~/.config/matugen/templates/nvim-dms.lua`; Matugen renders it to
`~/.config/nvim/colors/dms.lua`.

The full UI is transparent by default, including the Explorer, completion, and
pickers. Selected rows and the active buffer remain filled. A small contrast
guard lifts theme colors that are too dim against the current DMS background.
Use `:EditorTransparencyToggle` when an opaque UI is preferable.

Do not edit `colors/dms.lua` as the permanent source: the next Matugen run will
replace it.
