...
# Read layouts from the live Lua configuration.
kb_layout_value=$(hyprctl -j getoption input:kb_layout | jq -r '.str // empty')
if [[ -z "$kb_layout_value" ]]; then
  echo "Error: cannot read input:kb_layout from Hyprland" >&2
  exit 1
fi
IFS=',' read -r -a kb_layouts <<<"${kb_layout_value//[[:space:]]/}"
count=${#kb_layouts[@]}
