...
inc_mic_volume() {
    if [ "$(pamixer --default-source --get-mute)" == "true" ]; then
        ...
        if (( current_vol >= 26 && current_vol <= 29 )); then
            # Decide whether 25 or 30 is closer
            diff_to_25=$(( current_vol - 25 ))
            diff_to_30=$(( 30 - current_vol ))
            if (( diff_to_25 < diff_to_30 )); then
                new_vol=25
            else
                new_vol=30
            fi
            pamixer --default-source --set-volume "$new_vol" && notify_mic_user
        else
